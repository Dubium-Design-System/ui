...
const ExamplesDemo = () => {
	...
	const errorComponentStyle = useMemo<CSSProperties>(() => {
		return {
			display: "grid",
			placeItems: "center",
			minHeight: 180,
...
/>`,
	...
	/>
		...
		[errorComponentStyle, events, pushEvent],
	);
...
const meta = {
	...
	argTypes: {
		...
		aspectRatio: {
			control: "text",
			description: 'Соотношение сторон, например "16/9" или "auto".',
		},
		width: {
			control: "text",
			description: "Ширина контейнера изображения.",
		},
		height: {
			control: "text",
			description: "Высота контейнера изображения.",
		},
		objectFit: {
			control: { type: "select" },
			options: ["cover", "contain", "fill", "none", "scale-down"],
			description: "CSS object-fit для изображения.",
		},
		objectPosition: {
			control: "text",
			description: "CSS object-position для изображения.",
		},
		placeholder: {
			control: { type: "select" },
			options: ["empty", "blur"],
			description: "Тип placeholder.",
		},
		blurDataURL: {
			control: "text",
			description: "URL изображения для blur-placeholder.",
		},
		loading: {
			control: { type: "select" },
			options: ["lazy", "eager"],
			description: "Нативный loading для img.",
		},
		decoding: {
			control: { type: "select" },
			options: ["async", "sync", "auto"],
			description: "Нативный decoding для img.",
		},
		onLoad: {
			action: "loaded",
			description: "Callback успешной загрузки.",
		},
		onError: {
			action: "error",
			description: "Callback ошибки загрузки.",
		},
	},
...
export const CustomLoaderStory: Story = {
	name: "Custom loader",
	render: (args) => {
		return (
			<div style={{ width: 520 }}>
