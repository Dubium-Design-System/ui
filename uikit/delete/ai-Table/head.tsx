...
const Head = <T extends object>({ columns, manager }: { columns: Column<T>[]; manager?: TableManager<T> }) => (
	<div className={style.table__head}>
		{columns.map((col, i) => {
			const key = col.dataIndex ? String(col.dataIndex) : `${col.title}-${i}`
			...
			return (
				...
				>
					{manager ? <TableSort manager={manager} column={col} /> : col.title}
				</div>
			)
		})}
	</div>
