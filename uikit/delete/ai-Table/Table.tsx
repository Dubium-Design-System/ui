...
const TableComponent = <T extends { id?: string | number }>({
	...
	useSyncExternalStore(
		(onStoreChange) => (manager ? manager.subscribe(onStoreChange) : () => {}),
	...
	const getRowKey = (row: T): string | number => {
		if (rowKey === undefined) {
			...
			throw new Error("rowKey is required when 'id' field is not present in data objects")
		}
		...
		if (key === undefined || key === null) {
			throw new Error(`rowKey "${String(rowKey)}" returned null/undefined`)
		}
	...
	const visibleColumns = useMemo(() => {
		...
		return columns.filter((col) => !col.dataIndex || !hidden.includes(String(col.dataIndex)))
	}, [columns, manager])
	...
	const columnTemplate = useMemo(
		() => visibleColumns.map((col) => (col.width ? `${col.width}px` : "1fr")).join(" "),
		[visibleColumns],
	)
	...
	const renderedData = useMemo(() => {
		...
		const sortCol = columns.find((c) => c.dataIndex && String(c.dataIndex) === sort.columnKey)
	...
	return (
		<div ref={containerRef} className={style.table_container}>
			<div className={style.table} style={{ gridTemplateColumns: columnTemplate }}>
				...
				<div className={clsx(style.table__body)}>
					{renderedData.length > 0 ? (
						renderedData.map((row) => {
							const key = getRowKey(row)
							return <TableRow key={String(key)} row={row} rowIndex={key} columns={visibleColumns} />
						})
					) : (
						<div style={{ gridColumn: "1 / -1", padding: "10px" }}>No Data</div>
					)}
				</div>
			</div>

			{containerWidth !== undefined && <div style={{ position: "absolute", width: containerWidth }} />}
		</div>
	)
}

const shallowEqualArrays = (arrA: unknown[], arrB: unknown[]): boolean =>
	arrA.length === arrB.length && arrA.every((item, index) => item === arrB[index])
...
) as <T extends { id?: string | number }>(props: TablePropsWithSettings<T>) => JSX.Element
