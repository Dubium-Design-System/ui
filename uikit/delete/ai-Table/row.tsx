...
const alignToJustify = (align: Align | undefined): React.CSSProperties["justifyContent"] => {
	switch (align ?? "left") {
		case "left":
			return "flex-start"
		case "center":
	...
	return (
		<div className={clsx(style.table__body_row)}>
			{columns.map((col, colIndex) => {
				const value = col.dataIndex ? (row[col.dataIndex] as TDataIndex) : undefined
				...
				const cellKey = col.dataIndex ? String(col.dataIndex) : String(colIndex)
...
const areRowPropsEqual = <T extends object>(prev: RowProps<T>, next: RowProps<T>): boolean => {
...
export const TableRow = memo(Row, areRowPropsEqual) as <T extends object>(props: RowProps<T>) => JSX.Element
