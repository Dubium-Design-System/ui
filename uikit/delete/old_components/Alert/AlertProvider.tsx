...
import { AlertContext, type AlertOptions, type AlertType } from "./AlertContext"

type Position = "top-left" | "top-center" | "top-right" | "bottom-left" | "bottom-right"
...
export const AlertProvider = ({ children, position = "top-right", defaultDuration = 3000 }: AlertProviderProps) => {
	// Валидация входных параметров
	if (defaultDuration <= 0) {
		console.warn("defaultDuration must be a positive number. Using default value 3000ms.")
		defaultDuration = 3000
	}

	const [alerts, setAlerts] = useState<{ message: string; id: number; timeout: number; type: AlertType }[]>([])
	const timeoutsRef = useRef<{
		[key: number]: ReturnType<typeof setTimeout>
	}>({})
	const [adaptivePosition, setAdaptivePosition] = useState<Position>(position)
	...
	const addAlert = ({ message, type = "success", timeout }: AlertOptions) => {
		...
		try {
			...
			if (alertTimeout <= 0) {
				console.warn("Alert timeout must be a positive number. Using default duration.")
				alertTimeout = defaultDuration
			}

			setAlerts((prev) => [{ message, id, timeout: alertTimeout, type }, ...prev])
	...
	const getPositionStyle = (): CSSProperties => {
		try {
			switch (adaptivePosition) {
				...
				default:
					console.warn(`Unknown position: ${adaptivePosition}. Using top-right.`)
					return { top: "10px", right: "10px" }
			}
		} catch (error) {
