...
const PREVIEW_BLUR = "https://ux-journal.ru/wp-content/uploads/2021/05/Screenshot-2021-05-30-at-22.30.09.png"
...
const AccordionItem = ({ title, description, code, isOpen, wasOpened, onToggle, render }: AccordionItemProps) => {
	const reactId = useId()
		...
		() => [
			{
				...
				description: "Минимальный пример. Компонент Image монтируется только после открытия аккордиона.",
				code: `<Image
...
/>`,
			...
			},
			{
				...
				description: "Старый webpSrc заменён на универсальный sources. JPG остаётся fallback.",
				code: `<Image
...
/>`,
			...
			},
			{
				...
				description: "srcSet можно передавать массивом. Компонент сам нормализует его в строку.",
				code: `<Image
...
/>`,
			...
			},
			{
				...
				description: "Разные изображения для mobile/desktop. Открой пример и измени ширину viewport.",
				code: `<Image
...
/>`,
			...
			},
			{
				...
				description: "Подходит для аватаров, иконок и изображений фиксированного размера.",
				code: `<Image
...
/>`,
			...
			},
			{
				...
				description: "Показывает кастомный loader, пока изображение загружается.",
				code: `<Image
...
/>`,
			...
			},
			{
				...
				description: "Демонстрация errorComponent. Здесь src намеренно сломан.",
				code: `<Image
...
/>`,
			...
			},
			{
				...
				description: "className/style относятся к контейнеру. imgClassName/imgStyle относятся к img.",
				code: `<Image
...
/>`,
			...
			},
			{
				...
				description: "Неконфликтующие props уходят напрямую в img через ...imgProps.",
				code: `<Image
...
/>`,
			...
			},
			{
				...
				description: "Callback-и вызываются только после монтирования Image внутри открытого аккордиона.",
				code: `<Image
...
/>`,
					...
					<Grid>
						...
						>
							...
							{events.length > 0 ? (
								...
								<p style={{ margin: "8px 0 0", color: "#666" }}>Пока событий нет</p>
							)}
						</div>
					</Grid>
				),
			},
			{
				...
				description: 'Для декоративных изображений передаём alt="" и aria-hidden.',
				code: `<Image
...
/>`,
			...
			},
			{
				...
				description: "width и height относятся к контейнеру компонента, а не к нативным атрибутам img.",
				code: `<Grid>
	<Image
	...
	return (
		...
		>
			...
			>
				...
				>
					Каждый пример находится внутри аккордиона. Пока аккордион закрыт, компонент <code>Image</code> не
					монтируется и изображение не начинает загружаться.
				</p>
			</header>
